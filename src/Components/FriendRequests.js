...
handleReject(e){
  ...
  render() {
    ...
    if (this.props.user) {
      requests = this.props.user.incoming_friend_requests.map(request => {return <div className="notifications"><Link to={`/users/${request.user.id}`}><h3 className="normal_text">{request.user.name}</h3></Link><p className="normal_text">{request.user.description}</p><button type="submit" onClick = {this.handleAccept} className="event_button btn" value={request.id}>Accept</button><button type="submit" value={request.id} onClick = {this.handleReject} className="event_button btn heightbuffersmall">Reject</button></div>}) 
    }
